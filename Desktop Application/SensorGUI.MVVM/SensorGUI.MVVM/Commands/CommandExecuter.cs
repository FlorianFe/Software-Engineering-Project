...
namespace commands
{
    public class CommandExecuter
    {
        ...
        public CommandExecuter(ObserverCollection observerCollection, System.IO.Ports.SerialPort serialPort1, System.IO.Ports.SerialPort serialPort2)
        {
        ...
        public void execute(SimpleCommand command)
        {
            try
            {
                command.execute(this.observerCollection);
                this.observerCollection.onModelUpdate();
            }
            catch (Exception e)
            {
        ...
        public void executeOnPort1(ReactiveHalfCommand command)
        {
            ...
            catch (Exception e)
            {
                ...
                Console.Write(e.Message + "\n");
                this.observerCollection.onErrorThrown(e.Message);
            }
        }
        ...
        public void notifyOnPort1(char[] answerData)
        {
            ...
            if (command.isCorrectAnswerFormat(answerData))
            {
                try
                {
                    command.react(answerData, this.observerCollection);
